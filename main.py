...
def Strategy_candle(prices):
    ...
    pattern=[]
    df = candlestick.doji(prices, target='doji')
    ...
    for i in range(0,len(df['bullish_harami'])):

        if df['bullish_harami'].iloc[i]==True:
            pattern.append('bullish_harami')
        elif df['doji'].iloc[i]==True:
            pattern.append('doji')
        elif df['bullish_engulfing'].iloc[i]==True:
            pattern.append('bullish_engulfing')
        elif df['bearish_engulfing'].iloc[i]==True:
            pattern.append('bearish_engulfing')
        elif df['morning_star'].iloc[i]==True:
            pattern.append('morning_star')
        elif df['doji_star'].iloc[i]==True:
            pattern.append('doji_star')
        elif df['bearish_harami'].iloc[i]==True:
...
def Mt5_backTest(muldhon,Current_time,window,totalTime):
    ...
    #
    #
    #     rates = mt5.copy_rates_range("EURUSDm", mt5.TIMEFRAME_M5, pt - timedelta(minutes=720), pt)
    #
    #
    #
    #     ticks_frame1 = pd.DataFrame(rates)
    #     #par=parabolic_sar(ticks_frame1, acceleration=0.02, maximum=0.2)
    #     #k,d=stochastic_oscillator(ticks_frame1, k_period=14, d_period=3)
    #     #print(par)
    #
    #     patt=Strategy_candle(ticks_frame1)
    #
    #     print(patt)
    #
    #     fig, ax = plt.subplots(2, 2)
...
if __name__ == '__main__':
    #Mt5()
    Mt5_backTest(5000,datetime.now()-timedelta(hours=12),10,timedelta(minutes=120))
